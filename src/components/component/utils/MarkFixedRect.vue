<template>
  <g>
    <circle v-for="(item ,index) in fixPoints"
        :key="index"
        v-show="activeRect>-1?true:false"
        :cx="item.fixedX*scale" :cy="item.fixedY*scale" r="3"
...
export default {
  props: [ 'fixPoints', 'activeRect', 'scale' ],
  data () {
    return {}
  },
  methods: {
    changeRectPointStart (index) {
      vm.$emit('dragStart', index)
    },
    changeRectPointEnd () {
      vm.$emit('dragEnd')
    }
